...
help:
	@echo "StayNest â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for StayNest via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get staynest-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=staynest-tools" --grpc-web
