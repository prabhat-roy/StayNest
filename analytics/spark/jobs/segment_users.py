...
spark = SparkSession.builder.appName("staynest-segment").getOrCreate()
df = spark.read.format("iceberg").load("staynest_mart.user_features")
...
preds.select("user_id","prediction").writeTo("staynest_mart.user_segments").using("iceberg").createOrReplace()
