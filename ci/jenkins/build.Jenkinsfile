pipeline {
    ...
    environment {
        REGISTRY = 'harbor.staynest.internal'
    }
    stages {
        ...
        stage('Sign')  { steps { sh 'cosign sign --yes ${REGISTRY}/staynest/${SERVICE}:${GIT_COMMIT}' } }
        stage('Push')  { steps { sh 'make push' } }
    }
    post {
        failure { slackSend channel: '#staynest-ci', message: "Build failed: ${env.BUILD_URL}" }
    }
}
